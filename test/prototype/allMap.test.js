...
describe('Promise.all with Map', () => {
  ...
  test('should resolve with a Map of results when all promises resolve', async () => {
    ...
    const result = await Promise.resolve().all(inputMap);
  ...
  test('should reject when any promise is rejected', async () => {
    ...
    await expect(Promise.resolve().all(inputMap)).rejects.toBe('Error');
  });
  ...
  test('should throw TypeError if input is not a Map', () => {
    expect(() => Promise.resolve().all(42)).toThrow(TypeError);
