...
const {sequence} = require('../../dist/clean');

describe('Promise.resolve().sequence', () => {
  test('should resolve with the results of async functions', async () => {
    const result = Promise.resolve().then(sequence([
      () => Promise.resolve(1),
      () => Promise.resolve(2),
      () => Promise.resolve(3),
    ]));
  ...
  test('should throw an error if any promise is rejected and skipIfError is false', async () => {
    await expect(Promise.resolve().then(sequence([
      () => Promise.resolve(1),
      () => Promise.reject(new Error('Error!')),
      () => Promise.resolve(3),
  ...
  test('should handle an empty array', async () => {
    const result = Promise.resolve().then(sequence([], { skipIfError: false }));
    expect(result instanceof Promise).toBe(true);
  ...
  test('should call async functions sequentially', async () => {
    ...
    const result = Promise.resolve().then(sequence(asyncFunctions));
