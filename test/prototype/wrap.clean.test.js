...
describe('promise.then(wrap)', () => {
  test('should resolve with the result of the callback with no arguments', async () => {
    ...
    const promise = Promise.resolve().then(wrap(asyncFunction));
  ...
  test('should handle if the async function do not use callback as 1st parameter', async () => {
    ...
    const promise = Promise.resolve().then(wrap((cb) => asyncFunction(1,'Hello',cb)));
    expect(promise instanceof Promise).toBe(true);
    for(let i = 0; i < 100; i++) {
      jest.runOnlyPendingTimers();
      await Promise.resolve();
  ...
  test('should throw TypeError if the first argument is not a function', () => {
    expect(() => Promise.resolve().then(wrap(123))).toThrow(TypeError);
    expect(() => Promise.resolve().then(wrap(null))).toThrow(TypeError);
    expect(() => Promise.resolve().then(wrap({}))).toThrow(TypeError);
    expect(() => Promise.resolve().then(wrap('string'))).toThrow(TypeError);
  });
