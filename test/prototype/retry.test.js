...
test('retry(asyncFunction)', async () => {
  ...
  const firstPromise = Promise.resolve().retry(asyncFunction);
  ...
  const secondPromise = Promise.resolve().retry(asyncFunction);
...
test('retry(asyncFunction, 10)', async () => {
  ...
  retryCount = 0;
  const failPromise = Promise.resolve().retry(asyncFunction);
  for(let i = 0; i < 100; i++) {
    jest.runOnlyPendingTimers();
    await Promise.resolve();
  }
  ...
  retryCount = 0;
  const promise = Promise.resolve().retry(asyncFunction, 10);
...
test('retry(asyncFunction, 10, 200)', async () => {
  ...
  const promise = Promise.resolve().retry(asyncFunction, 10, 200);
  expect(promise instanceof Promise).toBe(true);
  for(let i = 0; i < 100; i++) {
    jest.runOnlyPendingTimers();
    await Promise.resolve();
...
test('retry(promise)', async () => {
  expect(async () => await Promise.resolve().retry(Promise.reject(`Error`))).rejects.toMatch(`Error`);

  const promise = Promise.resolve().retry(Promise.resolve('a'));
  expect(promise instanceof Promise).toBe(true);
  expect(await promise).toBe('a');
})

test('retry(promise, 5)', async () => {
  expect(async () => await Promise.resolve().retry(Promise.reject(`Error`), 5)).rejects.toMatch(`Error`);

  const promise = Promise.resolve().retry(Promise.resolve('a'), 5);
  expect(promise instanceof Promise).toBe(true);
  expect(await promise).toBe('a');
})

test('retry(promise, 5, 10)', async () => {
  expect(async () => await Promise.resolve().retry(Promise.reject(`Error`), 5, 10)).rejects.toMatch(`Error`);

  const promise = Promise.resolve().retry(Promise.resolve('a'), 5, 10);
  expect(promise instanceof Promise).toBe(true);
  expect(await promise).toBe('a');
})
