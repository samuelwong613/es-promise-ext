...
export default function reduce<I,R,V0,V1,V2,V3,V4,V5,V6,V7,V8,V9> (
...
) : ReduceFunction<R>
{
  ...
  if (!Array.isArray(asyncFunctions)){
    throw new TypeError('Promise.prototype.reduce must be called with an async function array');
  }
  ...
  return (async () => {
    ...
    return result as unknown as Promise<Awaited<R>>;
  }) as ReduceFunction<R>;
}